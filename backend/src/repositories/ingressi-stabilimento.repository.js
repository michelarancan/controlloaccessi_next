...
function findAllByData(idSede, data, callback) {
    const query = `SELECT i.id, per.nome, per.cognome, i.badge AS idBadge, b.codice AS badge, i.targa, DATE_FORMAT(i.data_ingresso, '%H:%i:%s %d/%m/%Y') AS dataIngresso, DATE_FORMAT(i.data_uscita, '%H:%i:%s %d/%m/%Y') AS dataUscita, DATE_FORMAT(i.data_ingresso, '%H:%i:%s') as oraIngresso, i.categoria AS idCategoria, c.codice AS categoria, i.persona_riferimento AS idPersonaRiferimento, CONCAT(pr.cognome, ' ', pr.nome) AS personaRiferimento, pe.azienda AS idAzienda, a.ragione_sociale as azienda, i.divisione_destinazione AS idDivisione, d.nome as divisione 
